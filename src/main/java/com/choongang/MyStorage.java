...
public class MyStorage {
    ...
    // products는 현재 등록된 제품을 저장하는 배열입니다.
    // 여기에 '사이다' EMPTY는 상수
    static String[] products = new String[]{EMPTY,EMPTY,EMPTY,EMPTY,EMPTY};
    // counts는 제품의 수량을 저장하는 배열입니다.
    // product의 0번째 요소가 counts의 0번째 요소랑 매치
    static int[] counts = new int[]{0, 0, 0, 0, 0};

    // 예) products 첫 배열에 사이다가 있다면 counts 수량을 입력 가능
    // 등록을 취소하면 count에 해당하는 똑같은 인덱스 값을 취소하게 =0으로 되게 만들어야해

    // 예외가 나서 프로그램이 종료가 나지만 않으면 됨 그래서 에러메세지 처리해야해 [Error] 수량은 숫자로 입력해야 합니다.
    // 4번은 3번 참고해서 하면 돼
    ...
    static void prod_input(Scanner s){
        ...
        // TODO:
        System.out.println("제품 등록을 원하는 제품명을 입력하세요 : ");
        // 제품명 입력 받기
    ...
    static void prod_remove(Scanner s){
        ...
        System.out.println("제품 등록 취소를 원하는 제품명을 입력하세요 : ");
        // 제품명 입력 받기
        String productName = s.nextLine();
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        // TODO:
        // 문자열 먼저 받고, 문자열 검증해서, 숫자말고 다른게 있다면 return;
        ...
        int cnt = Integer.parseInt(str);  // 수량 입력 받기
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        System.out.println("[System] 차감할 수량을 입력해주세요 : ");
        // 숫자가 아닌 경우, 에러 메시지
        ...
        String REGEX = "[0-9]+";
        if (str.matches(REGEX)) {
            System.out.println("[Clear] 정상적으로 제품의 수량 차감이 완료되었습니다.");
        } else {
            System.out.println("[Error] 수량은 숫자로 입력해야 합니다.");
            return;
        }
